...
function apply(ctx) {
    ...
    ctx.command('votemanager.create <programName:string> <options:text>', '创建投票项目').alias('创建投票项目')
        ...
        await ctx.database.get('vote_manager_table', { programName: [programName] })
            ...
            if (typeof (row[0]) !== "undefined") {
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `【${programName}】项目名已存在,创建失败`);
            }
            else {
                let counts = {}, optionsSpit = options.split(' ');
                for (let item of optionsSpit) {
                    counts[item] = 0;
                }
                ctx.database.create('vote_manager_table', { programName: programName, counts: counts, owner: session.userId });
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `【${programName}】已创建`);
            }
        });
    });
    ctx.command('votemanager.delete <programName:string>', '删除投票项目').alias('删除投票项目')
        ...
        await ctx.database.get('vote_manager_table', { programName: [programName] })
            ...
            if (typeof (row[0]) !== "undefined") {
                ...
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `输入投票项目名称不存在`);
            }
        })
            .then(result => {
            if (result.owner !== session.userId) {
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `非投票项目创始人无权删除`);
            }
            else {
                ctx.database.remove('vote_manager_table', { programName: [programName] });
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `【${programName}】已删除`);
            }
        });
    });
    ctx.command('votemanager.clear', '清除投票项目', { authority: 5 }).alias('清除投票项目')
        ...
        session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `所有投票项目已被删除`);
    });
    ctx.command('votemanager.show', '展示投票项目').alias('展示投票项目')
        .example('展示投票项目')
        .action(async ({ session }) => {
        let row = await ctx.database.get('vote_manager_table', {}, ['programName']), messageList = [(0, koishi_1.h)('message', `所有投票项目：`)], i = 1;
        for (let item of row) {
            messageList.push((0, koishi_1.h)('message', `${i}.${item.programName}`));
            i++;
        }
    ...
    ctx.command('votemanager.vote <programName:string> <options:string>', '投票').alias('投票')
        ...
        await ctx.database.get('vote_manager_table', { programName: [programName] })
            ...
            .then(result => {
            if (typeof (result.counts[options]) !== "undefined") {
                return result;
            }
            else {
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `输入选项名称不存在`);
            }
        })
            .then(result => {
            let participant = result.participant, counts = result.counts;
            if (participant.indexOf(session.userId) !== -1) {
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `已在该项目中投票,投票失败`);
            }
            else {
                counts[options] += 1;
                ...
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `成功为【${programName}】中的选项【${options}】投入一票`);
            }
        });
    });
    ctx.command('votemanager.state <programName:string>', '展示投票情况').alias('展示投票情况')
        ...
        await ctx.database.get('vote_manager_table', { programName: [programName] }, ['counts'])
            ...
            if (typeof (row[0]) === "undefined") {
                ...
                let messageList = [(0, koishi_1.h)('message', `【${programName}】投票情况如下：`)], counts = row[0].counts;
                for (let item in counts) {
                    messageList.push((0, koishi_1.h)('message', `${item}:${counts[item]}`));
                }
                session.send((0, koishi_1.h)('message', { forward: true }, messageList));
            }
        });
    ...
    ctx.command('votemanager.add <programName:string> <options:string>', '增加投票选项').alias('增加投票选项')
        ...
        await ctx.database.get('vote_manager_table', { programName: [programName] }, ['counts'])
            ...
            if (typeof (counts[options]) !== "undefined") {
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `输入选项名称已存在`);
            }
            else {
                counts[options] = 0;
                ctx.database.set('vote_manager_table', { programName: programName }, { counts: counts });
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `成功为【${programName}】添加选项【${options}】`);
            }
        });
    });
    ctx.command('votemanager.reduce <programName:string> <options:string>', '删除投票选项').alias('删除投票选项')
        ...
        await ctx.database.get('vote_manager_table', { programName: [programName] }, ['counts', 'owner'])
            ...
            .then(result => {
            let owner = result.owner, counts = result.counts;
            if (owner !== session.userId) {
                ...
                ctx.database.set('vote_manager_table', { programName: programName }, { counts: counts });
                session.send((0, koishi_1.h)('quote', { id: session.messageId }) + `成功为【${programName}】删除选项【${options}】`);
            }
        });
    });
}
